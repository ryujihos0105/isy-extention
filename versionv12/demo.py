# 이 파일은 Triple Fusion 모델 추론 데모의 진입점입니다 (v12: RGB + FFT + CLIP)
# 사용법 (단일): python demo.py --image samples/real_001.jpg
# 사용법 (폴더): python demo.py --folder ../demo/fakes
# OOD 테스트  : python demo.py --folder <OOD 이미지 폴더>   → 장당 판정 + 전체 요약 출력
...
# 폴더 안의 이미지 파일 경로를 모두 수집하는 함수 (하위 폴더 포함)
...
def predict_one(model, device: str, image_path: str) -> dict:
    ...
    x_rgb, x_fft, x_clip, crop_status = preprocess_image(image_path)
    x_rgb  = x_rgb.to(device)
    x_fft  = x_fft.to(device)
    x_clip = x_clip.to(device)

    with torch.no_grad():
        logits = model(x_rgb, x_fft, x_clip)   # (1, 2)
...
def run_single(model_path: str, image_path: str) -> None:
    device = os.environ.get("SIMCAP_DEVICE", "cuda" if torch.cuda.is_available() else "cpu")
...
# 폴더 전체 이미지 배치 추론 모드 실행 (OOD 테스트용)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SimCap Triple Fusion 추론 데모 (v12: RGB + FFT + CLIP)")
    ...
    parser.add_argument("--folder", type=str, default=None,       help="폴더 경로 (전체 추론 / OOD 테스트)")
    parser.add_argument("--device", type=str, choices=["auto", "cpu", "cuda"], default="auto", help="추론 디바이스")
    args = parser.parse_args()

    if args.device == "auto":
        os.environ["SIMCAP_DEVICE"] = "cuda" if torch.cuda.is_available() else "cpu"
    else:
        os.environ["SIMCAP_DEVICE"] = args.device
